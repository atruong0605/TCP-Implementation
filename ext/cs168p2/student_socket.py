...
class StudentUSocket(StudentUSocketBase):
  ...
  def rx(self, p):
    ...
    * Send pending ACKs and FINs
    ...
    ood_detected = False
    while not ood_detected and not self.rx_queue.empty():
      ...
      if s |GT| self.rcv.nxt:
        ...
        if len(data) > 0:
          self.handle_accepted_seg(seg, p.app[self.rcv.nxt |MINUS| s:])
  ...
  def check_ack(self, seg):
    ...
    if self.state in (ESTABLISHED, FIN_WAIT_1, FIN_WAIT_2, CLOSE_WAIT, CLOSING, LAST_ACK):
      ## Start of Stage 4.1 ##
      if seg.ack |GE| snd.una:
        if seg.ack |GE| snd.nxt:
  ...
  def maybe_send(self):
    ...
    ## Start of Stage 4.3 ##
    remaining = min(len(self.tx_data), snd.wnd)
    while remaining > 0:
      payload = self.tx_data[:min(remaining, self.mss)]
      p = self.new_packet(data=payload)
      self.tx(p)
      self.tx_data = self.tx_data[min(remaining, self.mss):]
      remaining -= min(remaining, self.mss)
      num_pkts += 1
